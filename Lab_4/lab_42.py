...
for ind in range(len_file_list):
    file_name = Config.file_list[ind]
    zno_year = int(file_name[5:9])
    if check_coll.count_documents({'file_name': file_name}) == 0:
        check_coll.insert_one({'file_name': file_name, 'rows_count': 0, 'end_file': False})


    inserted_rows_count = coll.count_documents({'zno_year': zno_year})
    import_data(file_name, inserted_rows_count)
t2 = time.time()
